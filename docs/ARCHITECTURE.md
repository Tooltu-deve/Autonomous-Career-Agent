...
- **`cv-agent-service` sở hữu bảng `cv_generations`** và đảm nhiệm hai vai trò: (1) **consumer** nghe `cv.requested` → đọc profile từ Postgres theo `user_id` + JD, nhét vào prompt → LLM sinh CV (dùng feedback nếu là retry) → upsert `cv_generations` (`edit_status=draft`) → publish `cv.generated`; (2) **read/update API** (`GET/PUT /cvs/{id}`) phục vụ CV Editor (`edit_status` chuyển `edited` khi user lưu).
