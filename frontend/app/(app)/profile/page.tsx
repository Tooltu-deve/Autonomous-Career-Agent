...
type ActiveModal = "basic" | "summary" | "projects" | "education" | "skills" | null;
...
export default function MasterProfilePage() {
  ...
  useEffect(() => {
    ...
    async function loadAllData() {
      try {
        ...
        const [profRes, prefRes] = await Promise.allSettled([getProfile(), getPreferences()]);
        const prof: ProfileResponse | null = profRes.status === "fulfilled" ? profRes.value : null;
        const pref: PreferencesResponse | null = prefRes.status === "fulfilled" ? prefRes.value : null;
        ...
        setProfile({
          ...
          experiences: prof?.experiences?.map((exp) => ({
            id: exp.id,
            name: exp.title,
            description: exp.description || "",
            server: { organization: exp.organization, start_date: exp.start_date, end_date: exp.end_date },
          })) ?? [],
          education: prof?.educations?.map((edu) => ({
            id: edu.id,
            university: edu.school,
            degree: edu.degree || "",
            server: { field_of_study: edu.field_of_study, start_date: edu.start_date, end_date: edu.end_date, description: edu.description },
          })) ?? [],
          skills: prof?.skills?.map((s) => s.skill_name) ?? [],
          preferences: {
            targetRole: pref?.target_role || "",
            workType: pref?.remote_preference ? pref.remote_preference.toUpperCase() : "",
  ...
  const saveProfile = useCallback(async (patch: Partial<{
    ...
    setSaving(true);
    try {
      const mergedHeadline = patch.headline !== undefined ? patch.headline : profile.headline;
      const mergedLocation = patch.location !== undefined ? patch.location : profile.location;
      const mergedPhone = patch.phone !== undefined ? patch.phone : profile.phone;
      const mergedGithub = patch.github !== undefined ? patch.github : profile.github;
      const mergedLinkedin = patch.linkedin !== undefined ? patch.linkedin : profile.linkedin;
      const mergedTemplate = patch.preferredTemplate ?? profile.preferredTemplate;
      const mergedSummary = patch.summary !== undefined ? patch.summary : profile.summary;
      ...
      await putProfile({
        ...
        experiences: mergedExp.filter(e => e.name.trim()).map((e, i) => ({
          title: e.name.trim(),
        ...
        educations: mergedEdu.filter(e => e.university.trim()).map((e, i) => ({
      ...
      <div className={s.profileHeaderCard}>
        ...
        <div className={s.headerContent}>
          <div className={s.profileAvatarGroup}>
            <div className={s.largeAvatar}>
              ...
              <div className={s.editAvatarBadge} title="Edit Personal Information" onClick={() => setActiveModal("basic")}>
            ...
            <div className={s.profileTitleDetails}>
              ...
              <div className={s.profileHeadline}>{profile.headline || "Headline not updated"}</div>
        ...
        <div className={s.completionStrip}>
          <div className={s.completionInfo}>
            ...
            <span className={s.compText}>Your Master Profile is directly connected to the AI Agent system!</span>
          </div>
          <div className={s.compBarContainer}>
            <div className={s.compBarFill} style={{ width: `${completionPercent}%` }} />
          </div>
        </div>
      </div>
        ...
        <div className={s.profileMainCol}>
          ...
          <div className={s.sectionCard}>
            <div className={s.cardHeader}>
              ...
              <button className={s.btnAddItem} onClick={() => setActiveModal("summary")} type="button">
                + Add / Edit
              </button>
            </div>
            <p style={{ fontSize: "14px", lineHeight: 1.7, color: "var(--ink-muted)" }}>
              {profile.summary || "No summary provided — click 'Edit' to add one."}
            </p>
          </div>
          ...
          <div className={s.sectionCard}>
            <div className={s.cardHeader}>
              ...
              <button className={s.btnAddItem} onClick={() => setActiveModal("projects")} type="button">
                + Add / Edit
              </button>
            </div>
            ...
            <div className={s.projectGrid}>
              {profile.experiences.length > 0 ? (
                ...
                <div style={{ color: "#888", fontSize: "13px", gridColumn: "1/-1" }}>
                  No projects added yet — click &apos;+ Add / Edit&apos; to add one.
                </div>
              )}
            </div>
          </div>
          ...
          <div className={s.sectionCard}>
            <div className={s.cardHeader}>
              ...
              <button className={s.btnAddItem} onClick={() => setActiveModal("education")} type="button">
                + Add / Edit
              </button>
            </div>
              ...
              {profile.education.length > 0 ? (
                profile.education.map((edu) => (
                  <div key={edu.id} className={s.timelineItem}>
                    <div className={s.itemLogo} style={{ background: "var(--surface-2)" }}>🎓</div>
                ...
                <div style={{ color: "#888", fontSize: "13px" }}>
                  No education entries — click &apos;+ Add / Edit&apos; to add one.
                </div>
              )}
            </div>
          </div>
        ...
        <div className={s.profileSideCol}>
          ...
          <div className={s.sectionCard}>
            <div className={s.cardHeader}>
              ...
              <button
                ...
                type="button"
                style={{ fontSize: "12px", fontWeight: 600, color: "var(--primary-hover)", background: "none", border: "none", cursor: "pointer" }}
              >
                Edit
              </button>
            </div>
            <div className={s.skillChipsWrap}>
              {profile.skills.length === 0 && <span className={s.prefVal}>No skills added yet.</span>}
              {profile.skills.map((skill) => (
                <span key={skill} className={s.masterSkillPill}>{skill}</span>
              ))}
            </div>
          </div>
          ...
          <div className={s.sectionCard}>
            <div className={s.cardHeader}>
              ...
              <Link href="/profile-preferences" style={{ fontSize: "12px", fontWeight: 600, color: "var(--primary-hover)" }}>
                Edit
              </Link>
            </div>
            <div className={s.prefItemRow}>
              <span className={s.prefLbl}>Target Role:</span>
              <span className={s.prefVal}>{profile.preferences.targetRole || "Not updated"}</span>
            </div>
            <div className={s.prefItemRow}>
              <span className={s.prefLbl}>Work Type:</span>
              <span className={s.prefVal}>{profile.preferences.workType || "Not selected"}</span>
            </div>
            <div className={s.prefItemRow}>
          ...
          {/* ATS Health Card */}
          <div className={s.sectionCard} style={{ background: "linear-gradient(135deg, #FFFDFD 0%, #FAF6F6 100%)", borderColor: "#F5C7C5" }}>
            ...
            <p style={{ fontSize: "13px", color: "var(--ink-muted)", marginTop: "8px", lineHeight: 1.5 }}>
              ...
              <strong>{completionPercent}% ATS compliance standard</strong>. Information is fully connected between Profile Setup and Career Preferences.
            </p>
          </div>
        </div>
      </div>
    ...
    <div className={s.modalBackdrop} onClick={handleBackdrop}>
      <div className={s.modalDialog}>
        <div className={s.modalHeader}>
          ...
          <button className={s.modalClose} onClick={onClose} type="button" aria-label="Close">✕</button>
        </div>
        <div className={s.modalBody}>{children}</div>
        <div className={s.modalFooter}>
          <button className={s.modalBtnCancel} onClick={onClose} type="button" disabled={saving}>Cancel</button>
          <button className={s.modalBtnSave} onClick={onSave} type="button" disabled={saving}>
...
/* ═══════════════════════════════════════════════════
   Basic Info Modal
═══════════════════════════════════════════════════ */
function BasicInfoModal({ initialProfile, saving, onSave, onClose }: {
  ...
  const [preferredTemplate, setPreferredTemplate] = useState<TemplateName>(initialProfile.preferredTemplate);

  return (
    <ModalWrapper
      title="👤 Personal Information & CV Template"
      onClose={onClose}
      onSave={() => onSave({ headline, location, phone, github, linkedin, preferredTemplate })}
      saving={saving}
    >
...
/* ═══════════════════════════════════════════════════
   Summary Modal
═══════════════════════════════════════════════════ */
function SummaryModal({ initialValue, saving, onSave, onClose }: {
  initialValue: string;
  saving: boolean;
  ...
  return (
    <ModalWrapper title="Executive Summary" onClose={onClose} onSave={() => onSave(value)} saving={saving}>
      <div className={s.formGroup}>
...
═══════════════════════════════════════════════════ */
function ProjectsModal({ initialProjects, saving, onSave, onClose }: {
  ...
  const [projects, setProjects] = useState<ProjectEntry[]>(
    initialProjects.length > 0 ? initialProjects : [{ id: uid(), name: "", description: "" }]
  );

  const addProject = () => setProjects(p => [...p, { id: uid(), name: "", description: "" }]);
  const removeProject = (id: any) => setProjects(p => p.filter(x => x.id !== id));
  const updateProject = (id: any, field: "name" | "description", val: string) =>
    setProjects(p => p.map(x => x.id === id ? { ...x, [field]: val } : x));

  return (
    <ModalWrapper title="📁 Projects & Experience" onClose={onClose} onSave={() => onSave(projects)} saving={saving}>
      <ProjectsForm
        projects={projects}
...
═══════════════════════════════════════════════════ */
function EducationModal({ initialEducation, saving, onSave, onClose }: {
  ...
  const [education, setEducation] = useState<EducationEntry[]>(
    initialEducation.length > 0 ? initialEducation : [{ id: uid(), university: "", degree: "" }]
  );

  const addEdu = () => setEducation(e => [...e, { id: uid(), university: "", degree: "" }]);
  const removeEdu = (id: any) => setEducation(e => e.filter(x => x.id !== id));
  const updateEdu = (id: any, field: "university" | "degree", val: string) =>
    setEducation(e => e.map(x => x.id === id ? { ...x, [field]: val } : x));

  return (
    <ModalWrapper title="🎓 Education" onClose={onClose} onSave={() => onSave(education)} saving={saving}>
      <EducationForm
        education={education}
        onAdd={addEdu}
        onRemove={removeEdu}
...
═══════════════════════════════════════════════════ */
function SkillsModal({ initialSkills, saving, onSave, onClose }: {
  ...
  const removeSkill = (name: string) => setSkills(s => s.filter(x => x !== name));
  ...
  return (
    <ModalWrapper title="⭐ Master Skills" onClose={onClose} onSave={() => onSave(skills)} saving={saving}>
      <SkillsForm
        skills={skills}
        customSkill={custom}
