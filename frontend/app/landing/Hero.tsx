  ...
  return (
    <>
        ...
        <div className={`${landingStyles.wrap} ${landingStyles["hero-grid"]}`}>
          <div>
            ...
            <div className={landingStyles["hero-ctas"]}>
              <Link
                className={landingStyles["btn-primary-lg"]}
                href="/signin?tab=register"
              >
                Get Started for Free
              </Link>
              <span className={landingStyles["hero-subcta"]}>
          ...
          <div className={landingStyles["radar-wrap"]} aria-hidden="true">
            <i
              className={`${landingStyles["radar-ring"]} ${landingStyles.r3}`}
            />
            <i
              className={`${landingStyles["radar-ring"]} ${landingStyles.r2}`}
            />
            <i
              className={`${landingStyles["radar-ring"]} ${landingStyles.r1}`}
            />
            <i className={landingStyles["radar-pulse"]} />
            <span
              className={`${landingStyles["radar-node"]} ${landingStyles.n1}`}
            >
              <b />
              VNG — AI Developer
            </span>
            <span
              className={`${landingStyles["radar-node"]} ${landingStyles.n2}`}
            >
              <b />
              FPT — ML Intern
            </span>
            <span
              className={`${landingStyles["radar-node"]} ${landingStyles.n3}`}
            >
              <b />
              Axon — Backend
            </span>
            <span
              className={`${landingStyles["radar-node"]} ${landingStyles.n4}`}
            >
              <b />
              Grab — QA
            </span>
        ...
        <div className={landingStyles.marquee}>
          <div className={landingStyles["marquee-track"]}>
            {[...boards, ...boards].map(([name, color], index) => (
              ...
              >
                <i
                  className={[landingStyles.dot, landingStyles[color]]
                    .filter(Boolean)
                    .join(" ")}
                />
                {name}
      ...
      <section
        ...
        aria-label="CareerNav landing page preview"
      >
